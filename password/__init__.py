...
def very_weak():
    ...
    check50.run("./password pass").stdout("Very Weak\n").exit(0)
...
def fair():
    ...
    check50.run("./password 3l3phant!").stdout("Fair\n").exit(0)

@check50.check(compiles)
def strong():
    """checks for a strong password"""
    check50.run("./password :hlH|8i1I\4}").stdout("Strong\n").exit(0)

@check50.check(compiles)
def very_strong():
    """checks for a very strong password"""
    check50.run("./password 5k1bidi_Riz2-0HiO").stdout("Very Strong\n").exit(0)
